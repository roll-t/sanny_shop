...
class AddCart {
    ...
    renderCartItems(wrapList, value){
        ...
        const content = `
                    ...
                    <input value='${value.quantity}' type="hidden" class="quanity-cart-items">
                    <input value='${value.size}' type="hidden" class="size-cart-items">
    ...
    checkingProduct() {
        const listProduct = document.querySelectorAll('.cart-items')
        listProduct.forEach(items => {
            if (items.querySelector('.check-confirm').checked) {
                const id=items.querySelector(".id-cart-items")
                const quantity=items.querySelector('.quanity-cart-items')
                const size=items.querySelector('.size-cart-items')
                id.name='id[]'
            ...
            listProductCart.forEach((items, index) => {
                ...
                check.addEventListener('change', e => {
                    ...
                    if (checkInput) {
                        ...
                        console.log(this.checkingProduct())
                    } else {
